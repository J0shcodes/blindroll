 ...
 */
async function decryptSalary(contractAddress: string, signer: SignerWithAddress, handle: string): Promise<bigint> {
 ...
 */
async function decryptErrorCode(contractAddress: string, signer: SignerWithAddress, handle: string): Promise<bigint> {
  return fhevm.userDecryptEuint(FhevmType.euint8, handle, contractAddress, signer);
}
...
const SALARY_ALICE = ethers.parseEther("1.0"); // $5,000
const SALARY_BOB = ethers.parseEther("1.7"); // $8,500
const SALARY_CAROL = ethers.parseEther("1.5"); // $3,200

// ETH amounts for the plaintext mirror (withdrawal accounting)
const SALARY_ALICE_ETH = SALARY_ALICE;
const SALARY_BOB_ETH = SALARY_BOB;
const SALARY_CAROL_ETH = SALARY_CAROL;
...
const TREASURY_DEPOSIT_UNITS = TREASURY_DEPOSIT_ETH; // Encrypted mirror amount
...
describe("Blindroll", function () {
  ...
  describe("Deployment", function () {
    ...
    it("starts with treasury uninitialized", async function () {
      await expect(blindroll.connect(employer).getEncryptedTreasuryBalance()).to.be.revertedWith("Treasury not funded");
    });
    ...
    it("employer address is immutable — cannot be changed", async function () {
      ...
      const { handle, proof } = await encryptSalary(contractAddress, employer, SALARY_ALICE_ETH);
  ...
  describe("addEmployee", function () {
    it("employer can add an employee with an encrypted salary", async function () {
      ...
      await expect(blindroll.connect(employer).addEmployee(alice.address, handle, proof))
    ...
    it("an unauthorized address cannot decrypt another employee's salary", async function () {
      ...
      await expect(decryptSalary(contractAddress, bob, salaryHandle)).to.be.rejected;
    });
    ...
    it("stranger cannot decrypt an employee's salary", async function () {
      ...
      await expect(decryptSalary(contractAddress, stranger, salaryHandle)).to.be.rejected;
    });
    ...
    it("reverts if the same employee is added twice", async function () {
      ...
      await expect(blindroll.connect(employer).addEmployee(alice.address, h2, p2))
    ...
    it("reverts if called by a non-employer", async function () {
      const { handle, proof } = await encryptSalary(contractAddress, employer, SALARY_ALICE);
      await expect(blindroll.connect(stranger).addEmployee(alice.address, handle, proof)).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployer",
      );
    });
  ...
  describe("updateSalary", function () {
    ...
    it("employer can update an employee's salary", async function () {
      ...
      await expect(blindroll.connect(employer).updateSalary(alice.address, handle, proof))
        .to.emit(blindroll, "SalaryUpdated")
        .withArgs(alice.address, await getBlockTimestamp());
    });
    ...
    it("reverts if employee does not exist", async function () {
      ...
      await expect(blindroll.connect(employer).updateSalary(bob.address, handle, proof))
  ...
  describe("deactivateEmployee / reactivateEmployee", function () {
    ...
    it("reverts deactivate if employee not found", async function () {
      await expect(blindroll.connect(employer).deactivateEmployee(stranger.address)).to.be.revertedWithCustomError(
        blindroll,
        "EmployeeNotFound",
      );
    });
    ...
    it("reverts reactivate if employee not found", async function () {
      await expect(blindroll.connect(employer).reactivateEmployee(stranger.address)).to.be.revertedWithCustomError(
        blindroll,
        "EmployeeNotFound",
      );
    });
    ...
    it("non-employer cannot deactivate", async function () {
      await expect(blindroll.connect(stranger).deactivateEmployee(alice.address)).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployer",
      );
    });
  ...
  describe("depositToTreasury", function () {
    ...
    it("employer can decrypt the treasury balance after deposit", async function () {
      const { handle, proof } = await encryptSalary(contractAddress, employer, TREASURY_DEPOSIT_ETH);
    ...
    it("stranger cannot decrypt the treasury balance", async function () {
      ...
      const treasuryHandle = await blindroll.connect(employer).getEncryptedTreasuryBalance();
      await expect(decryptSalary(contractAddress, stranger, treasuryHandle)).to.be.rejected;
    });

    it("reverts if deposit amount is zero", async function () {
      const { handle, proof } = await encryptSalary(contractAddress, employer, TREASURY_DEPOSIT_UNITS);
      await expect(blindroll.connect(employer).depositToTreasury(handle, proof, { value: 0n })).to.be.revertedWith(
        "Deposit must be non-zero",
      );
    });
  ...
  describe("executePayroll", function () {
    ...
    beforeEach(async function () {
      ...
      await blindroll
        .connect(employer)
        .depositToTreasury(treasuryInput.handle, treasuryInput.proof, { value: TREASURY_DEPOSIT_ETH });
    });
    ...
    it("employee cannot decrypt another employee's balance", async function () {
      ...
      await expect(decryptSalary(contractAddress, alice, bobHandle)).to.be.rejected;
    });
    ...
    it("deactivated employees are skipped during payroll", async function () {
      ...
      await expect(blindroll.connect(bob).getMyEncryptedBalance()).to.be.revertedWith("No balance yet");
    ...
    it("reverts if treasury has not been funded", async function () {
      ...
      await expect(fresh.connect(employer).executePayroll()).to.be.revertedWith("Treasury not funded");
    });

    it("non-employer cannot execute payroll", async function () {
      await expect(blindroll.connect(stranger).executePayroll()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployer",
      );
    });
  ...
  describe("Safe Payroll — Insufficient Treasury (FHE.select pattern)", function () {
    it("does not revert when treasury cannot cover all salaries — uses FHE.select", async function () {
      ...
      await blindroll
        .connect(employer)
        .depositToTreasury(treasuryInput.handle, treasuryInput.proof, { value: ethers.parseEther("0.1") });

      // This must NOT revert — FHE.select() silently zeroes the payment
      await expect(blindroll.connect(employer).executePayroll()).to.not.be.reverted;
    });
  ...
  describe("withdraw", function () {
    beforeEach(async function () {
      ...
      // We use SALARY_ALICE_ETH (1.0 ETH) so the units match the ETH transfer
      const aliceInput = await encryptSalary(contractAddress, employer, SALARY_ALICE_ETH);
      await blindroll.connect(employer).addEmployee(alice.address, aliceInput.handle, aliceInput.proof);
      await blindroll.connect(employer).setPlainSalaryMirror(alice.address, SALARY_ALICE_ETH);

      const treasuryInput = await encryptSalary(contractAddress, employer, TREASURY_DEPOSIT_ETH);
      await blindroll
        .connect(employer)
        .depositToTreasury(treasuryInput.handle, treasuryInput.proof, { value: TREASURY_DEPOSIT_ETH });
    ...
    it("employee can withdraw their accumulated ETH balance using public decryption", async function () {
      const aliceBalanceBefore = await ethers.provider.getBalance(alice.address);

      // --- PHASE 1: On-Chain Request ---
      // This marks the balance publicly decryptable
      const reqTx = await blindroll.connect(alice).requestWithdrawal();
      const reqReceipt = await reqTx.wait();
      const reqGas = reqReceipt!.gasUsed * reqReceipt!.gasPrice;

      // --- PHASE 2: Off-Chain Decryption ---
      const balanceHandle = (await blindroll.connect(alice).getMyEncryptedBalance()) as `0x${string}`;
      const results = await fhevm.publicDecrypt([balanceHandle]);

      const decryptedAmount = results.clearValues[balanceHandle] as bigint;
      const decryptionProof = results.decryptionProof as `0x${string}`;

      // Verify the decrypted amount is 1.0 ETH (SALARY_ALICE_ETH)
      expect(decryptedAmount).to.equal(SALARY_ALICE_ETH);

      // --- PHASE 3: On-Chain Execution ---
      const execTx = await blindroll.connect(alice).executeWithdraw(decryptedAmount, decryptionProof);
      const execReceipt = await execTx.wait();
      const execGas = execReceipt!.gasUsed * execReceipt!.gasPrice;

      // Verify the ETH transfer (Alice receives SALARY_ALICE_ETH minus gas for both txs)
      const aliceBalanceAfter = await ethers.provider.getBalance(alice.address);
      expect(aliceBalanceAfter).to.equal(aliceBalanceBefore + SALARY_ALICE_ETH - reqGas - execGas);
    });

    it("reverts if a fake or invalid decryption proof is provided", async function () {
      await blindroll.connect(alice).requestWithdrawal();

      // Attacker tries to submit a fake proof
      const fakeProof = ethers.hexlify(ethers.randomBytes(64));

      await expect(blindroll.connect(alice).executeWithdraw(SALARY_ALICE_ETH, fakeProof)).to.be.reverted;
    });

    it("encrypted balance is zeroed after a successful withdrawal", async function () {
      await blindroll.connect(alice).requestWithdrawal();
      ...
      await blindroll
        .connect(alice)
        .executeWithdraw(results.clearValues[balanceHandle] as bigint, results.decryptionProof as `0x${string}`);

      // After withdrawal, encrypted balance should be zero
      const newBalanceHandle = await blindroll.connect(alice).getMyEncryptedBalance();
      const decrypted = await decryptSalary(contractAddress, alice, newBalanceHandle);
    ...
    it("employee cannot double-withdraw — second withdrawal is zero", async function () {
      // First withdrawal
      await blindroll.connect(alice).requestWithdrawal();
      let balanceHandle = (await blindroll.connect(alice).getMyEncryptedBalance()) as `0x${string}`;
      let results = await fhevm.publicDecrypt([balanceHandle]);
      await blindroll
        .connect(alice)
        .executeWithdraw(results.clearValues[balanceHandle] as bigint, results.decryptionProof as `0x${string}`);
      ...
      balanceHandle = (await blindroll.connect(alice).getMyEncryptedBalance()) as `0x${string}`;
      results = await fhevm.publicDecrypt([balanceHandle]);

      const execTx = await blindroll
        .connect(alice)
        .executeWithdraw(results.clearValues[balanceHandle] as bigint, results.decryptionProof as `0x${string}`);
      const execReceipt = await execTx.wait();

      const totalGas = reqReceipt!.gasUsed * reqReceipt!.gasPrice + execReceipt!.gasUsed * execReceipt!.gasPrice;
      const balanceAfter = await ethers.provider.getBalance(alice.address);

      // Balance decreases only by gas (no ETH received)
      expect(balanceAfter).to.equal(balanceBefore - totalGas);
    });

    it("non-employee cannot call requestWithdrawal or executeWithdraw", async function () {
      await expect(blindroll.connect(stranger).requestWithdrawal()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployee",
      );

      const fakeProof = ethers.hexlify(ethers.randomBytes(64));
      await expect(blindroll.connect(stranger).executeWithdraw(100n, fakeProof)).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployee",
      );
    });
  ...
  describe("Access Control", function () {
    it("non-employee cannot call getMyEncryptedSalary", async function () {
      await expect(blindroll.connect(stranger).getMyEncryptedSalary()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployee",
      );
    });
    ...
    it("non-employee cannot call getMyEncryptedBalance", async function () {
      await expect(blindroll.connect(stranger).getMyEncryptedBalance()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployee",
      );
    });
    ...
    it("non-employer cannot call getEncryptedTreasuryBalance", async function () {
      ...
      await expect(blindroll.connect(alice).getEncryptedTreasuryBalance()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployer",
      );
    });
    ...
    it("employer cannot call getMyEncryptedSalary (not an employee)", async function () {
      await expect(blindroll.connect(employer).getMyEncryptedSalary()).to.be.revertedWithCustomError(
        blindroll,
        "NotEmployee",
      );
    });
  ...
  describe("setPlainSalaryMirror", function () {
    ...
    it("employer can set the plaintext salary mirror", async function () {
      ...
      await expect(blindroll.connect(employer).setPlainSalaryMirror(alice.address, SALARY_ALICE_ETH)).to.not.be
        .reverted;
    });
    ...
    it("mirror can be updated (salary raise scenario)", async function () {
      ...
      await expect(blindroll.connect(employer).setPlainSalaryMirror(alice.address, newMirror)).to.not.be.reverted;
    });
  ...
  describe("Encrypted Error Codes", function () {
    ...
    it("ErrorChanged event is emitted for each employee during payroll", async function () {
      ...
      await expect(blindroll.connect(employer).executePayroll())
        .to.emit(blindroll, "ErrorChanged")
        .withArgs(alice.address)
        .and.to.emit(blindroll, "ErrorChanged")
        .withArgs(bob.address);
    });
  ...
  describe("Edge Cases", function () {
    it("getMyEncryptedBalance reverts before first payroll", async function () {
      ...
      await expect(blindroll.connect(alice).getMyEncryptedBalance()).to.be.revertedWith("No balance yet");
    });
    ...
    it("salary of maximum euint64 value can be set without overflow issues", async function () {
      ...
      await expect(blindroll.connect(employer).addEmployee(alice.address, handle, proof)).to.not.be.reverted;
  ...
  describe("End-to-End: Full Payroll Lifecycle", function () {
    it("complete lifecycle: onboard → fund → payroll → update salary → payroll → withdraw", async function () {
      ...
      const newAliceSalary = ethers.parseEther("1.3");
      const newAliceETH = ethers.parseEther("1.3");
      const aliceRaiseInput = await encryptSalary(contractAddress, employer, newAliceSalary);

      await blindroll.connect(employer).updateSalary(alice.address, aliceRaiseInput.handle, aliceRaiseInput.proof);
      ...
      // ── Step 6: Alice withdraws (v0.9 3-step process) ─────────────────────
      ...
      // Phase 3
      const withdrawTx = await blindroll
        .connect(alice)
        .executeWithdraw(results.clearValues[balanceHandle] as bigint, results.decryptionProof as string);
      const withdrawReceipt = await withdrawTx.wait();
      const execGas = withdrawReceipt!.gasUsed * withdrawReceipt!.gasPrice;
      ...
      expect(aliceEthAfter).to.equal(aliceEthBefore + expectedAliceEth - reqGas - execGas);
      ...
      await expect(decryptSalary(contractAddress, bob, aliceSalaryHandle)).to.be.rejected;
